module.exports = {
  ...
  description: 'Mettre à jour le mot de passe du transporteur connecté.',
  ...
  fn: async function ({ currentPassword, newPassword }) {
    ...
    const passwordsMatch = await bcrypt.compare(currentPassword, carrier.password);
    if (!passwordsMatch) {
      throw {
        badCombo: {
    ...
    try {
      await Carrier.updateOne({ id: this.req.me.id }).set({ password: newPassword });
    } catch (err) {
      if (err.message) {
