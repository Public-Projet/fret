  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="modelValue"
        ...
        <div
          class="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800 transform transition-all">
          <!-- Header -->
          <div class="relative p-6 text-center border-b border-gray-50 dark:border-gray-800">
            <button @click="close"
              class="absolute right-4 top-4 p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
              <IconX class="w-5 h-5" />
            </button>
            <div :class="[
          ...
          <div class="p-8">
            <div v-if="!success" class="space-y-6">
              <div class="space-y-2">
                ...
                <div class="relative group">
                  ...
                  <input id="resend-email" v-model="email" type="email"
                    ...
                    placeholder="exemple@email.com" @keyup.enter="handleResend" />
                </div>
              </div>

              <div v-if="error"
                class="p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30">
            ...
            <div v-else class="text-center space-y-6 py-4">
              <div
                class="w-20 h-20 mx-auto rounded-full bg-green-100 dark:bg-green-900/30 text-green-600 flex items-center justify-center shadow-inner animate-bounce">
...
const handleResend = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      success.value = true;
      successMessage.value = extractSuccessMessage(result.data, 'Un nouveau lien de vérification a été envoyé.');
    } else {
