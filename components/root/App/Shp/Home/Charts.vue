<template>
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-[2rem] p-6 lg:p-8 shadow-xl border border-gray-100 dark:border-gray-700/50 relative overflow-hidden">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h3 class="text-xl font-black text-gray-900 dark:text-white">Analyse des coûts</h3>
          <p class="text-xs text-gray-400 uppercase tracking-wider font-bold mt-1">Évolution mensuelle du budget de fret</p>
        </div>
        <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-900/50 p-1.5 rounded-xl border border-gray-100 dark:border-gray-800">
          <button 
            v-for="tab in ['Trimestre', 'Année']" 
            :key="tab"
      ...
      <div class="relative w-full h-64 mt-6">
        <svg viewBox="0 0 500 200" class="w-full h-full overflow-visible">
          ...
          <line x1="0" y1="40" x2="500" y2="40" stroke="currentColor" class="text-gray-100 dark:text-gray-700/50" stroke-dasharray="4,4"/>
          <line x1="0" y1="90" x2="500" y2="90" stroke="currentColor" class="text-gray-100 dark:text-gray-700/50" stroke-dasharray="4,4"/>
          <line x1="0" y1="140" x2="500" y2="140" stroke="currentColor" class="text-gray-100 dark:text-gray-700/50" stroke-dasharray="4,4"/>
        ...
        >
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ chartData[hoveredIndex].month }}</p>
          <p class="text-sm font-black text-emerald-400 mt-0.5">{{ (chartData[hoveredIndex].budget).toLocaleString() }} FCFA</p>
          <p class="text-[10px] font-medium text-blue-300">{{ chartData[hoveredIndex].volume }} tonnes acheminées</p>
        </div>
      </div>

      <!-- Labels X -->
      <div class="flex justify-between px-2 mt-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
        <span v-for="pt in chartData" :key="pt.month">{{ pt.month }}</span>
      </div>
    </div>

    <!-- Répartition des Chargements (Donut Interactif) -->
    <div class="bg-white dark:bg-gray-800 rounded-[2rem] p-6 lg:p-8 shadow-xl border border-gray-100 dark:border-gray-700/50 flex flex-col justify-between">
      <div>
      ...
      <div class="relative flex items-center justify-center py-6">
        <svg width="180" height="180" viewBox="0 0 100 100" class="transform -rotate-90">
          <!-- Fond gris -->
          <circle cx="50" cy="50" r="40" fill="transparent" stroke="currentColor" class="text-gray-100 dark:text-gray-700/50" stroke-width="12" />
          
          <!-- Terminés (Vert) -->
          <circle cx="50" cy="50" r="40" fill="transparent" stroke="#10b981" stroke-width="12" 
            :stroke-dasharray="251.2" 
            :stroke-dashoffset="dashOffsets.completed"
            stroke-linecap="round"
          ...
          <circle cx="50" cy="50" r="40" fill="transparent" stroke="#f59e0b" stroke-width="12" 
            :stroke-dasharray="251.2" 
            :stroke-dashoffset="dashOffsets.negotiating"
            :transform="`rotate(${angles.negotiating} 50 50)`"
            stroke-linecap="round"
            class="transition-all duration-1000"
          />
          ...
          <circle cx="50" cy="50" r="40" fill="transparent" stroke="#3b82f6" stroke-width="12" 
            :stroke-dasharray="251.2" 
            :stroke-dashoffset="dashOffsets.pending"
            :transform="`rotate(${angles.pending} 50 50)`"
            stroke-linecap="round"
            class="transition-all duration-1000"
          />
        </svg>
        ...
        <div class="absolute flex flex-col items-center justify-center">
          ...
          <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Marchandises</span>
        </div>
      </div>

      <!-- Légende stylisée -->
      <div class="grid grid-cols-3 gap-2 mt-2">
        <div class="flex flex-col items-center p-2.5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800">
        ...
        <div class="flex flex-col items-center p-2.5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800">
          ...
          <span class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">En attente</span>
          <span class="text-sm font-black text-gray-800 dark:text-white mt-0.5">{{ activeAnnouncementsCount - negotiatingCount }}</span>
        </div>
      </div>
    </div>
  </div>
