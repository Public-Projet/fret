  ...
  <div class="min-h-screen bg-gray-50/50 dark:bg-gray-900/50 pb-20 pt-10">
    <div class="container-custom">
      ...
      <NuxtLink to="/app/uc/vehicles"
        ...
        <div
          class="p-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 mr-3 group-hover:bg-secondary-50 dark:group-hover:bg-secondary-900/30 transition-colors">
      ...
      <div v-else-if="error"
        ...
        <NuxtLink to="/app/uc/vehicles"
          class="inline-flex px-8 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-all active:scale-95 shadow-lg shadow-red-500/20">
          Retour à la liste
        </NuxtLink>
      </div>
          ...
          <div
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-8">
              <div class="flex items-center">
                ...
                <div>
                  <h1 class="text-3xl lg:text-4xl font-black text-gray-900 dark:text-white tracking-tight">{{
                    vehicle.brand }} {{ vehicle.model }}</h1>
                  <div
                    class="inline-flex items-center px-3 py-1 bg-gray-100 dark:bg-gray-700/50 rounded-lg border border-gray-100 dark:border-gray-700 mt-2">
              ...
              <div class="flex flex-wrap gap-3 w-full md:w-auto">
                <button @click="openStatusModal"
                  class="flex-1 md:flex-none px-5 py-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-all active:scale-95 flex items-center justify-center">
                  <IconRefresh class="w-4 h-4 mr-2" /> Statut
                </button>
                <button @click="openEditModal"
                  class="flex-1 md:flex-none px-5 py-3 rounded-xl bg-primary-600 text-white font-bold text-sm hover:bg-primary-700 transition-all shadow-lg shadow-primary-500/20 active:scale-95 flex items-center justify-center">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-10 border-t border-gray-50 dark:border-gray-700/50 pt-10">
              <div class="group">
                <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Type de véhicule</h3>
                <div
                  class="flex items-center p-4 bg-gray-50 dark:bg-gray-900/30 rounded-2xl border border-gray-100 dark:border-gray-800 group-hover:border-secondary-200 transition-all">
              ...
              <div class="group">
                ...
                <div
                  :class="['flex items-center p-4 rounded-2xl border transition-all', getStatusClass(vehicle.status)]">
              ...
              <div class="group">
                ...
                <div
                  ...
                  <span class="text-lg font-black text-gray-900 dark:text-white">{{ vehicle.capacity || '-' }} <span
                      class="text-xs text-gray-400 uppercase ml-1">Tonnes</span></span>
                </div>
              </div>
              ...
              <div class="group">
                ...
                <div
                  ...
                  <span class="text-lg font-black text-gray-900 dark:text-white">{{ vehicle.volume || '-' }} <span
                      class="text-xs text-gray-400 uppercase ml-1">m³</span></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-8">
          ...
          <div v-if="vehicle.availability"
            ...
            <div class="flex items-center justify-between mb-8 relative">
              ...
              <span
                class="px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 text-[10px] font-black uppercase tracking-wider border border-emerald-100 dark:border-emerald-800/50 shadow-sm">Active</span>
            </div>
              ...
              <div class="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p class="text-sm font-bold text-gray-900 dark:text-white mt-1">
                    Du {{ formatDateShort(vehicle.availability.startDate) }} au {{
                      formatDateShort(vehicle.availability.endDate) }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p class="text-2xl font-black text-emerald-600 mt-1">
                    {{ vehicle.availability.price?.toLocaleString() || 'Sur devis' }} <span
                      v-if="vehicle.availability.price" class="text-sm font-bold ml-1">FCFA</span>
                  </p>
                </div>
              </div>
            </div>
              ...
              <div v-for="item in vehicle.history" :key="item.id"
                ...
                <div class="flex justify-between items-center mb-1">
                  <p class="text-sm font-black text-gray-900 dark:text-white">
                    {{ item.origin.city }}
                    <IconArrowRight class="w-3 h-3 text-gray-400 inline mx-1" /> {{ item.destination?.city || 'Partout'
                    }}
                  </p>
                  <span
                    class="px-2 py-0.5 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-400 text-[8px] font-black uppercase tracking-wider border border-gray-100 dark:border-gray-600">
                    {{ item.status }}
                  </span>
                </div>
          ...
          <div v-else-if="!vehicle.availability"
            ...
            <p class="text-gray-500 text-sm font-medium mb-8">Ce véhicule est actuellement hors ligne. Publiez son
              trajet pour être visible.</p>
...
import { IconArrowLeft, IconTruck, IconPencil, IconTrash, IconRefresh, IconMapPin, IconCalendar, IconCurrencyEuro, IconAlertCircle, IconWeight, IconPackage, IconSettings, IconArrowRight } from '@tabler/icons-vue';
...
const loadVehicle = async () => {
  ...
  const result = await carVehicleStore.fetchCarVehicle(vehicleId);
...
const handleStatusChange = async (newStatus: string) => {
  ...
  const result = await carVehicleStore.updateCarVehicleStatus(vehicleId, newStatus as 'available' | 'in_transit' | 'maintenance');
...
const handleSubmit = async (data: AddVehicleData) => {
  ...
  const result = await carVehicleStore.updateCarVehicleDetail(vehicleId, data);
...
const handleDelete = async () => {
  ...
  const result = await carVehicleStore.deleteCarVehicle(vehicleId);
  if (result.success) {
