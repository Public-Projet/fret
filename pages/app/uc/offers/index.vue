    ...
    <div
      ...
      <div class="p-6 border-b border-gray-100 dark:border-gray-700">
        <div class="flex space-x-2 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-hide">
          <button v-for="status in ['all', 'pending', 'negotiating', 'accepted', 'rejected', 'completed']" :key="status"
      ...
      <div v-else-if="filteredOffers.length === 0" class="p-20 text-center">
        ...
        <p class="mt-2 text-gray-500 dark:text-gray-400 max-w-sm mx-auto">Vous n'avez pas encore d'offres correspondant
          à ce filtre.</p>
          ...
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-3 mb-3">
                ...
                <span class="text-xs font-medium text-gray-400 tabular-nums">Offre #{{ String(offer.id || '').slice(0,
                  8) }}</span>
              </div>
              ...
              <div class="flex flex-col sm:flex-row sm:items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                ...
                <div class="flex items-center px-1">
                  ...
                  <span>Soumis le <span class="font-bold text-gray-700 dark:text-gray-300">{{
                    formatDate(offer.createdAt) }}</span></span>
                </div>
              </div>
            </div>
