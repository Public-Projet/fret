        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div v-for="item in filteredAvailabilities" :key="item.id"
            ...
            <div v-if="isNew(item.createdAt)" class="absolute top-6 right-6 z-10">
              <span class="bg-emerald-500 text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-emerald-500/30">Nouveau</span>
            </div>

            <div class="p-8">
              <div class="flex justify-between items-start mb-8">
                <div class="flex items-center space-x-5">
                  <div class="w-16 h-16 bg-primary-50 dark:bg-primary-900/30 rounded-2xl flex items-center justify-center text-primary-600 shadow-inner">
              ...
              <div class="flex flex-wrap gap-3 mb-8">
                <span class="px-4 py-2 bg-gray-50 dark:bg-gray-700/50 rounded-xl text-[11px] font-black text-gray-500 uppercase tracking-wider flex items-center border border-gray-100 dark:border-gray-700">
                ...
                <span class="px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl text-[11px] font-black text-emerald-600 uppercase tracking-wider flex items-center border border-emerald-100 dark:border-emerald-900/30">
              ...
              <div class="flex items-center justify-between border-t border-gray-50 dark:border-gray-700 pt-6">
                <div class="flex items-center">
                  <div class="w-10 h-10 rounded-full bg-primary-600 text-white font-black flex items-center justify-center mr-3 shadow-lg shadow-primary-500/20 group-hover:scale-110 transition-transform">
                    {{ item.carrier?.firstname?.[0] || 'C' }}
                  </div>
                  <div>
                    <span class="text-sm font-black text-gray-900 dark:text-white">{{ item.carrier?.firstname || 'Transporteur' }}</span>
      ...
      <div v-if="activeTab === 'enrollments'">
        ...
        <div v-else class="space-y-6">
          <div v-for="enrollment in enrollments" :key="enrollment.id"
            ...
            <div class="flex flex-col items-center md:items-end md:px-10 md:border-x border-gray-100 dark:border-gray-700 w-full md:w-auto py-4 md:py-0">
...
import {
  ...
  IconTruck, IconCalendar, IconPackage, IconUsers, 
  IconTruckOff, IconMapPin, IconChevronDown, IconArrowDown
