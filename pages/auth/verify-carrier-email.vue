      ...
      <div v-else-if="error" class="py-12">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <NuxtLink to="/auth/register?role=carrier"
            class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-secondary-600 hover:bg-secondary-700 text-white font-medium rounded-xl transition-all">
...
onMounted(async () => {
  ...
  try {
    const response = await api.post<{ message: string }>('/carrier/auth/verify-email', { token });
